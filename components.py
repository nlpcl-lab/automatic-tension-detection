...
class BucketedDataIterator():
    ...
    def next_batch(self, n):
        ...
        self.cursor[i] += n
        return np.asarray(res['doc'].tolist()), res['label'].tolist(), res['length'].tolist()
...
def visualize(sess, inputs, revlens, max_rev_length, keep_probs, index2word, alphas_words, alphas_sents, x_test, y_test, y_predict, visual_sample_index):
    ...
    if y_test_sample[0] != y_test_predict[0] or int(y_test_sample[0]) == 2:
        return

    print("index: {}, test sample is {}".format(visual_sample_index, y_test_sample[0]))
    ...
    with open(sents_visual_file, "w") as html_file:
        ...
        html_file.write('actual label: {}, predicted label: {}<br><br>'.format(get_text_label(y_test_sample[0]), get_text_label(y_test_predict[0])))
